...
def loan_interest_payment_list(request):
    ...
    if is_member(user):
        payments = LoanInterestPayment.objects.filter(loan__user=user).select_related('loan', 'loan__user')
    else:
        payments = LoanInterestPayment.objects.select_related('loan', 'loan__user')
